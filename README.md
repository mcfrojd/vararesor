...
- **Plats och Android:** Android tar bort GPS ur bilder som väljs med filväljaren i webbläsaren (fälten finns kvar men är tömda). Appen känner igen det och säger till. Bilder som i stället delas till appen från Google Foto (*Dela → Våra resor*, kräver att appen är installerad) behåller platsen: manifestet har ett `share_target`, och `static/share-target.js` i service workern tar emot bilderna och skickar dem till `/bilder/ladda-upp`. I bildvisningen står det *Plats från Doris spår* när positionen kommer från spåret och inte från bilden.
